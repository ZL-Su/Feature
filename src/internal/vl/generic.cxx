...
vl_configuration_to_string_copy ()
{
  ...
  while (string == 0) {
    if (length > 0) {
      string = static_cast<char*>(vl_malloc(sizeof(char) * length)) ;
...
vl_lock_state (void)
{
  ...
  //EnterCriticalSection (&vl_get_state()->mutex) ;
   std::lock_guard<std::mutex> guard(vl_get_state()->mutex);
#endif
#endif
}
...
#endif
  self = static_cast<VlThreadState*>(malloc(sizeof(VlThreadState))) ;
  self->lastError = 0 ;
...
vl_destructor ()
{
...
#elif defined(VL_THREADS_WIN)
 {
    /* Delete the thread state of this thread as the
       ...
       using the library, so this is fine.
     */
    auto threadState = static_cast<VlThreadState*>(TlsGetValue(state->tlsIndex)) ;
    if (threadState) {
      vl_thread_specific_state_delete (threadState) ;
