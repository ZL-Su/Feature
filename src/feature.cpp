...
Sift::keypoints_t Sift::detect(const image_f32_t data, bool verbose) const
{
	...
	if (edge_thresh >= 0) vl_sift_set_edge_thresh(f, edge_thresh);
	if (peak_thresh >= 0) vl_sift_set_peak_thresh(f, peak_thresh);
	if (magnif >= 0) vl_sift_set_magnif(f, magnif);
